...
public class HomeContinuousNestedTopAreaBehavior extends HomeViewOffsetBehavior<View> {
    ...
    public HomeContinuousNestedTopAreaBehavior(Context context) {
        this(context, null);
    }


    public HomeContinuousNestedTopAreaBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    void scroll(@NonNull CoordinatorLayout parent, @NonNull View child, int dy) {
        ...
        if (child instanceof HomeContinuousNestedTopView) {
            unConsumed = ((HomeContinuousNestedTopView) child).consumeScroll(unConsumed);
        }
    ...
    public void onNestedPreScroll(@NonNull CoordinatorLayout parent, @NonNull View child,
        ...
        if (target == child) {
            ...
            if (dy > 0) {
                ...
                if (target instanceof HomeContinuousNestedBottomView) {
                    int contentHeight = ((HomeContinuousNestedBottomView) target).getContentHeight();
                    int minOffset;
        ...
        if (target == child) {
            ...
            if (dyUnconsumed > 0) {
                ...
                if (bottomView == null || bottomView.getVisibility() == View.GONE) {
                    ...
                    int contentHeight = ((HomeContinuousNestedBottomView) bottomView).getContentHeight();
                    ...
                    if (canContentScroll) {
                        ((HomeContinuousNestedBottomView) bottomView).consumeScroll(dyUnconsumed);
                    }
                }
            }
        } else {
            ...
            if (dyUnconsumed < 0) {
                ...
                if (child instanceof HomeContinuousNestedTopView) {
                    ((HomeContinuousNestedTopView) child).consumeScroll(dyUnconsumed);
                }
            }
        }
    }
    ...
    private View findBottomView(CoordinatorLayout parent) {
        for (int i = 0; i < parent.getChildCount(); i++) {
            ...
            if (child instanceof HomeContinuousNestedBottomView) {
                return child;
            }
        }
        return null;
