...
import com.wisn.medial.finalview.layout.INotchInsetConsumer;
import com.wisn.medial.finalview.layout.IWindowInsetLayout;
...
public class HomeWindowInsetHelper {
    ...
    public HomeWindowInsetHelper(ViewGroup viewGroup, IWindowInsetLayout windowInsetLayout) {
        mWindowInsetLayoutWR = new WeakReference<>(windowInsetLayout);
        KEYBOARD_HEIGHT_BOUNDARY = HomeDisplayHelper.dp2px(viewGroup.getContext(), 100);

        if (HomeNotchHelper.isNotchOfficialSupport()) {
    ...
    public boolean defaultApplySystemWindowInsets19(ViewGroup viewGroup, Rect insets) {
        ...
        if (insets.bottom >= KEYBOARD_HEIGHT_BOUNDARY) {
            HomeViewHelper.setPaddingBottom(viewGroup, insets.bottom);
            ...
            HomeViewHelper.setPaddingBottom(viewGroup, 0);
        }
    ...
    public boolean defaultApplySystemWindowInsetsCompat(ViewGroup viewGroup, WindowInsetsCompat insets) {
        ...
        if (insets.getSystemWindowInsetBottom() >= KEYBOARD_HEIGHT_BOUNDARY) {
            showKeyboard = true;
            HomeViewHelper.setPaddingBottom(viewGroup, insets.getSystemWindowInsetBottom());
            viewGroup.setTag(R.id.qmui_window_inset_keyboard_area_consumer, KEYBOARD_CONSUMER);
        } else {
            HomeViewHelper.setPaddingBottom(viewGroup, 0);
            viewGroup.setTag(R.id.qmui_window_inset_keyboard_area_consumer, null);
        ...
        for (int i = 0; i < viewGroup.getChildCount(); i++) {
            ...
            if (HomeNotchHelper.needFixLandscapeNotchAreaFitSystemWindow(viewGroup) &&
                    viewGroup.getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
                insetLeft = Math.max(insetLeft, HomeNotchHelper.getSafeInsetLeft(viewGroup));
                insetRight = Math.max(insetRight, HomeNotchHelper.getSafeInsetRight(viewGroup));
            }
