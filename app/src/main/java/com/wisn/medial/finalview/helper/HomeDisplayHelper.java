...
package com.wisn.medial.finalview.helper;
 ...
 * @date 2016-03-17
 */
public class HomeDisplayHelper {
    ...
    private static final String TAG = "HomeDisplayHelper";
    ...
    public static int getScreenHeight(Context context) {
        ...
        if(HomeDeviceHelper.isXiaomi() && xiaomiNavigationGestureEnabled(context)){
    ...
    public static int[] getRealScreenSize(Context context) {
        if (HomeDeviceHelper.isEssentialPhone() && Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
    ...
    public static int getUsefulScreenWidth(Activity activity) {
        return getUsefulScreenWidth(activity, HomeNotchHelper.hasNotch(activity));
    }

    public static int getUsefulScreenWidth(View view) {
        return getUsefulScreenWidth(view.getContext(), HomeNotchHelper.hasNotch(view));
    }
    ...
    public static int getUsefulScreenWidth(Context context, boolean hasNotch) {
        ...
        if (!hasNotch) {
            if (isLandscape && HomeDeviceHelper.isEssentialPhone()
                ...
                result -= 2 * HomeStatusBarHelper.getStatusbarHeight(context);
            }
            return result;
        }
        if (isLandscape) {
            // 华为挖孔屏横屏时，会把整个 window 往后移动，因此，可用区域减小
            if (HomeDeviceHelper.isHuawei() && !HomeDisplayHelper.huaweiIsNotchSetToShowInSetting(context)) {
                result -= HomeNotchHelper.getNotchSizeInHuawei(context)[1];
            }
    ...
    *//**
    ...
    public static int getUsefulScreenHeight(Activity activity) {
        return getUsefulScreenHeight(activity, HomeNotchHelper.hasNotch(activity));
    }

    public static int getUsefulScreenHeight(View view) {
        return getUsefulScreenHeight(view.getContext(), HomeNotchHelper.hasNotch(view));
    }
    ...
    private static int getUsefulScreenHeight(Context context, boolean hasNotch) {
        ...
        if (!hasNotch) {
            if (isPortrait && HomeDeviceHelper.isEssentialPhone()
                    && Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
                // https://arstechnica.com/gadgets/2017/09/essential-phone-review-impressive-for-a-new-company-but-not-competitive/
                // 这里说挖孔屏是状态栏高度的两倍
                result -= 2 * HomeStatusBarHelper.getStatusbarHeight(context);
