...
import com.download.db.BreakpointStoreOnSQLite;
...
public class Util {
    ...
    DownloadStore createDefaultDatabase(Context context) {
        return  new  BreakpointStoreOnSQLite(context);
//        return new BreakpointStoreOnCache();
    ...
    public static @NonNull DownloadStore createRemitDatabase(@NonNull DownloadStore originStore) {
        if(originStore instanceof BreakpointStoreOnSQLite){
            originStore= ((BreakpointStoreOnSQLite) originStore).createRemitSelf();
        }

        Util.d("Util", "Get final download store is " + originStore);
        return originStore;
    }
